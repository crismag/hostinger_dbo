...
// The primary client can be defined entirely through the environment, which keeps
// secrets out of this file. GATEWAY_SECRET falls back to the older
// API_CLIENT_EXAMPLE_SECRET name for backward compatibility.
$gatewayClientId = getenv('GATEWAY_CLIENT_ID') ?: 'example-client';
$gatewaySecret = getenv('GATEWAY_SECRET')
    ?: (getenv('API_CLIENT_EXAMPLE_SECRET') ?: 'replace-with-a-long-random-secret');
...
return [
    ...
    // (GATEWAY_CLIENT_ID / GATEWAY_SECRET) so no credential is written to this file.
    'client_secrets' => [
        $gatewayClientId => $gatewaySecret,
    ],
